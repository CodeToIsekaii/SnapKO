-- SnapKO - Fix Audit Gap: Auto-Update Inventory Quantities
-- Creates trigger to update ingredients.warehouse_qty/bar_qty when inventory_logs inserted

-- Function to update ingredient quantities based on log type
...
-- Create trigger on inventory_logs
DROP TRIGGER IF EXISTS trg_update_ingredient_quantity ON public.inventory_logs;
CREATE TRIGGER trg_update_ingredient_quantity
  AFTER INSERT ON public.inventory_logs
  FOR EACH ROW
  EXECUTE FUNCTION public.update_ingredient_quantity();

-- Add comment for documentation
COMMENT ON FUNCTION public.update_ingredient_quantity() IS 
  'Automatically updates ingredients.warehouse_qty or bar_qty when inventory_logs are inserted. Handles IMPORT, TRANSFER, WASTE, LENT, and AUDIT log types.';
